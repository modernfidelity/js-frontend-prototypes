...
describe('MTRAK : ', function () {
    ...
    describe('Non-Authenticated Users', function () {
        ...
        it('Frontpage - section renders', function () {
            ...
            expect(element(by.id('hero-promo')).isPresent()).toBe(true);
        ...
        // Pre-owned SECTION
        it('Pre-owned - section renders', function () {

            // Tell not to check for Angular on this page
            browser.driver.get(webapp + urlPrefix + "/pre-owned");

            // Increase sleep time due data being pulled from PROD server
            browser.sleep(5000); // Sleep to make sure page loads fully..

            // Check Page Title
            expect(element(by.id('page-title')).getText()).toContain('Pre-owned');
